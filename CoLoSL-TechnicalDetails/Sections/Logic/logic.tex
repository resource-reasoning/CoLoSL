...
A pair $(\lmod, \gmod)$ of action models is \emph{closed} under a subjective state $s$, context $r$, and action model $\lmod'$, written $\extendsAM{\lmod, \gmod}{s}{r}{\lmod'}$, if $\lmod' \subseteq \lmod$ and $\extendsAMUpto{\lmod, \gmod}{n}{s}{r}{\lmod '}$ for all $n\in\Nats$, where $\downarrow_n$ is defined recursively as follows:
%%
%\[
%\begin{array}{@{} L @{}}
...
\begin{array}{@{} L @{}}
  ...
  \qquad\quad \neg\m{visible}(a,s) /| \for{(s', r') \in a[s, r]} \extendsAMUpto{\lmod, \gmod}{n}{s'}{r'}{\lmod'})\hfill \text{(4)}
...
The above states that the $(\lmod, \gmod)$ pair is closed under $(s, r, \lmod')$ if the closure relation holds for any number of steps $n \in \Nats$ where 
\begin{itemize}
...
\begin{itemize}
	...
	\begin{enumerate}\renewcommand{\theenumi}{\alph{enumi}}
		\item $a$ is reflected in $\lmod'$ ($a$ is known to the subjective view); \textit{or} \hfill(2)
		\item  \label{item:closure4}
		$a$ does not affect the subjective state $s$ ($a$ is not visible in $s$), and $(\lmod, \gmod)$ is also closed under any subjective state $s'$ and context $r'$ resulting from application of $a$ ($(s', r') \in a[s, r]$). \hfill (4)
	\end{enumerate}
\end{itemize}
%
%
...
\]
%
We first demonstrate that, whenever an action model pair is closed under a subjective state $s_1 \composeL s_2$ and context $r$, then it is also closed under the smaller subjective state $s_1$, and the larger context extended with the forgotten state. That is,
%
\[
