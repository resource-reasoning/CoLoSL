...
%
\vspace{-2ex}
\clearpage\section{Set Module}\label{sec:set}
...
\begin{array}{@{} l @{}}
	...
	\begin{array}{@{} r @{\hspace{2pt}}  l @{}}
		...
		\color{darkgreen}{\unlockT{x}[1]} : &  \color{blue}{ \left\{ \cell{x.lock}{1} \swap \cell{x.lock}{0} * \unlockC{x} \right\} }\\
		\color{darkgreen}{\lockT{x}[1] * \unlockT{x}[1]}:  &  \color{blue}{ \left\{ \cell{x.lock}{1} \swap \lockT{x}[1] * \unlockT{x}[1] \right\} }
	\end{array}
	\right\} \vspace{10pt}\\
...
%
\hrule
\caption{\colosl\ specification of the concurrent set module where the definition of $I(h, x)$ is provided in \fig~\ref{fig:set-interference}.}
...
\paragraph{\textsf{isLock}($x, \pi$) / \textsf{locked}($x$)} 
...
\paragraph{\textsf{U}($x, v, y$) / \textsf{L}($x, v, y$)}
...
%
When a thread successfully locks the node at $x$, it claims the next pointer, the modification capability and the locking capability pertaining to its successors ($\link{x}{y}$), and moves them into its local state. In return, it transfers the $\locked{x}$ resource to the shared state as evidence that it is indeed the locking thread.
...
%
%
\subsection*{\colosl vs. CAP Specification}
