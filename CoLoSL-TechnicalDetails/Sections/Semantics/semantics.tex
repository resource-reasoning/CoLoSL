\chapter{\colosl Program Logic}\label{sec:semantics}
...
The \emph{orthogonal} of a state  $\heapSize{.}: \Heaps \rightarrow \pset{\Heaps}$ is defined as the set of all states that are compatible with it:
%
\[
  	...
  	\begin{array}{@{} l @{} }
	  	...
	  	\begin{array}{@{} l @{} }
	 		g = g' \lor\\
	 	  	\left(
	 	  	\begin{array}{@{} l @{}}
	 	  		\exsts{\ca{} \containedIn \capPart{l}}
	 	  		(g, g') \in \gmod(\ca{}) \land\\
...
Our proof rules are of the form $\entails \{P\}\ C\ \{Q\}$ and carry an implicit assumption that the pre- and post-conditions of their judgements are stable. Most of the proof rules are standard and omitted for space reasons; they include the rules of concurrent separation logic~\cite{csl-tcs} (including the \proofRule{Parallel}
rule mentioned in the introduction). We use two additional rules, similar to their CAP counterparts~\cite{cap-ecoop10}, which are as follows, where $\entails_{\textsf{SL}}$ denotes the standard sequential separation logic judgement:
...
It suffices to show that the repartitioning ($\semimplies$) is valid and allowed by the guarantee relation. We can show that transitions corresponding to applications of \extendRule are contained in the \extendG\ relation.
\end{proof}
%
\end{lemma}
%
...
%
\[
  \left\{ \snd{\updateFP{a}} \composeL s' \ \middle|\ \exsts{r} \fst{\updateFP{a}}= p_s \composeL r /| s = p_s \composeL s' /| s \composeL r \text{ is defined}\; \right\}
\]
%
\end{definition}
...
\nested{
\begin{lemma}[]
	for all $P \in \Assertions$, $\lenv \in \LEnv$, $s, g \in \LStates$ and $\lmod, \gmod \in \AMods$:  
		\[
			s, \lenv |=_{g, \lmod, \gmod} P \implies s, \lenv \slsat P
		\]
%
\begin{proof}
...
%
\begin{align}
	\tag{I.H1} & s, \lenv |=_{g, \lmod, \gmod} P_1 \implies s, \lenv \slsat P_1 \label{subshift:IH1}\\ 
	\tag{I.H2} & s, \lenv |=_{g, \lmod, \gmod} P_2 \implies s, \lenv \slsat P_2 \label{subshift:IH2}\\
	& s, \lenv |=_{g, \lmod, \gmod} P_1 \implies P_2 \label{subshift:Ass1}
\end{align}
%
From (\ref{subshift:Ass1}) and the definition of $|=_{g, \lmod, \gmod}$ we know $s, \lenv |=_{g, \lmod, \gmod} P_1$ \text{ implies } $P_2 |=_{g, \lmod, \gmod}$; consequently, from (\ref{subshift:IH1}) and (\ref{subshift:IH2}) we have: $s, \lenv \slsat P_1$ \text{ implies } $s, \lenv \slsat P_2$. Thus, from the definition of $\slsat$ we have:
%
\begin{align*}
...
From \ref{shift:Ass4}, \ref{shift:Goal2} (established above), and the following \lem~ we have
%
\begin{align}
...
\nested{
\begin{lemma}[]
for all $P \in \Assertions$, $\lenv \in \LEnv$, $s, g \in \LStates$ and $\lmod', \lmod, \gmod \in \AMods$: 
% 
\[
\begin{array}{@{} l @{}}
	s, \lenv |=_{g, \lmod, \gmod} P /| \left(\for{s', \lmod_0} \extendsAM{\lmod, \gmod}{s'}{g-s'}{\lmod_0} \!\!\!\implies\!\!\! \extendsAM{\lmod \cup \lmod', \gmod}{s'}{g-s'}{\lmod_0} \right)   \\
	\qquad\quad \implies  s, \lenv |=_{g, \lmod \cup \lmod', \gmod} P
\end{array}
\]
%
\begin{proof}
...
\noindent\textbf{Case $P \eqdef A$\quad} Immediate.\\
\noindent\textbf{Case $P \eqdef P_1 \implies P_2$} \\
Pick an arbitrary $\lenv \in \LEnv$, $s, g \in \LStates$ and $\lmod', \lmod, \gmod \in \AMods$ such that
%
\begin{align}
	& s, \lenv |=_{g, \lmod, \gmod} P_1 \implies P_2 \label{subshift2:Ass1}\\
%	
	&\left(\for{s', \lmod_0} \extendsAM{\lmod, \gmod}{s'}{g-s'}{\lmod_0} \implies \extendsAM{\lmod \cup \lmod', \gmod}{s'}{g-s'}{\lmod_0} \right) \label{subshift2:Ass2}\\
%	
	\tag{I.H1} & 
	\hspace{-10pt} 
	\begin{array}{@{} l @{}}
		s, \lenv |=_{g, \lmod, \gmod} P_1 /| \left(\for{s', \lmod_0} \extendsAM{\lmod, \gmod}{s'}{g-s'}{\lmod_0} \!\!\implies\!\! \extendsAM{\lmod \cup \lmod', \gmod}{s'}{g-s'}{\lmod_0} \right)   \\
	 	\qquad \quad \implies s, \lenv |=_{g, \lmod \cup \lmod', \gmod} P_1
	\end{array}\label{subshift2:IH1}\\ 
%
	\tag{I.H2} & 
	\hspace{-10pt} 
	\begin{array}{@{} l @{}}
		s, \lenv |=_{g, \lmod, \gmod} P_2 /| \left(\for{s', \lmod_0} \extendsAM{\lmod, \gmod}{s'}{g-s'}{\lmod_0} \!\!\implies\!\! \extendsAM{\lmod \cup \lmod', \gmod}{s'}{g-s'}{\lmod_0} \right)   \\
	 	\qquad \quad \implies s, \lenv |=_{g, \lmod \cup \lmod', \gmod} P_2
	\end{array} \label{subshift2:IH2}
%	
\end{align}
%
From (\ref{subshift2:Ass1}) and the definition of $|=_{g, \lmod, \gmod}$ we know $s, \lenv |=_{g, \lmod, \gmod} P_1$ \text{ implies } $P_2 |=_{g, \lmod, \gmod}$; consequently, from (\ref{subshift2:Ass2}), (\ref{subshift2:IH1}) and (\ref{subshift2:IH2}) we have: $s, \lenv |=_{g, \lmod \cup \lmod', \gmod} P_1$ \text{ implies } $s, \lenv |=_{g, \lmod \cup \lmod', \gmod} P_2$. Thus, from the definition of $|=_{g, \lmod \cup \lmod', \gmod}$ we have:
%
\begin{align*}
...
\noindent\textbf{Case $P \eqdef \shared{P'}{I}$} \\
Pick an arbitrary $\lenv \in \LEnv$, $s, g \in \LStates$ and $\lmod_0, \lmod', \lmod, \gmod \in \AMods$ such that
%
\begin{align}
%	\tag{I.H} & 
%	\begin{array}{@{} l @{}}
%		\for{s, s', g \in \LStates} \for{\lmod_0, \lmod', \lmod, \gmod \in \AMods}\\
%		\quad s, \lenv |=_{g, \lmod, \gmod} P' /| \left(\extendsAM{\lmod, \gmod}{s'}{g-s'}{\lmod_0} \implies \extendsAM{\lmod \cup \lmod', \gmod}{s'}{g-s'}{\lmod_0} \right)  \implies \\
%	 	\qquad \quad s, \lenv |=_{g, \lmod \cup \lmod', \gmod} P'
%	\end{array} \label{subshift2:IH}\\ 
	&s, \lenv |=_{g, \lmod, \gmod} \shared{P'}{I} \label{subshift2:Ass3}\\
	& \for{s', \lmod_0} \extendsAM{\lmod, \gmod}{s'}{g-s'}{\lmod_0} \implies \extendsAM{\lmod \cup \lmod', \gmod}{s'}{g-s'}{\lmod_0} \label{subshift2:Ass4}
\end{align}
%
From (\ref{subshift2:Ass3}) and the definition of $|=_{g, \lmod, \gmod}$ we have:
%
\begin{align*}
	s = \unitL /| \exsts{s', r'} g = s' \composeL r' /| s', \lenv |=_{g, \lmod, \gmod} P' /| \extendsAM{\lmod, \gmod}{s'}{r'}{\semI[\lenv]{I}} 
\end{align*}  
%
Thus from (\ref{subshift2:Ass4}) we have
%
\begin{align*}
	s = \unitL /| \exsts{s', r'} g = s' \composeL r' /| s', \lenv |=_{g, \lmod, \gmod} P' /| \extendsAM{\lmod \cup \lmod', \gmod}{s'}{r'}{\semI[\lenv]{I}}
\end{align*}
%
That is,
%
...
%
\begin{lemma}
The semantic implications \extendRule\ is valid.
\begin{proof}
