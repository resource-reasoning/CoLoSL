...
\emph{cross-split} property\footnote{ A separation algebra
  ...
  $c = x + z$, and $d = y + w$.}\cite{dockins2009fresh}.  As we demonstrate in the examples of
...
$p$ and $q$ are the \emph{pre-} and \emph{post-states} of the action, and $c$ is its
...
have their standard classical meaning. Following the RGSep style~\cite{viktor-marriage}, interference assertions $I$ describe actions enabled by a given capability, in the form of a pre- and post-condition ($P$ and $Q$ respectively) as well as a side-condition ($R$). We write $P \swap Q$ as a shorthand for $\action{P}{Q}{\emp}$.
...
\semI[\lenv]{I}(\token{A}) &==
  ...
  \begin{array}{@{}l@{\ }|@{\ }r@{}}
    (p, q, c)&
    \begin{array}{@{}l@{}}
      \interAss{[\token{A}]}{\bar x}{P}{Q}{R} \in I /| \exsts{\bar{v}, \lmod, r, r'} \\
      \quad c = r \composeL r' /| r, [\lenv|||\bar x:\bar v]  |=_{c, \lmod} R \;/| \\
...
\begin{lemma}
  ...
  \forgetRule, \mergeRule, \shiftRule, \weakenRule, \copyRule are valid.
  %% \begin{mathpar}
  %%       \infer{
...
can be found in~\cite{colosl-tr14}.% Appendix~\ref{sec:assertions-continued}.
...
\begin{definition}[Precision, exactness]
...
require the presence of some neutral resource outside of $l'$ provided that they are conserved by the action (i.e.\ are also present in $q$).
%
...
because its pre- and side-conditions is incompatible with $\lass{F}$: no state
...
\begin{mathpar}
	...
	{
          \for{\left(\interAss{[\token{A}]}{\bar x}{\lass{P}}{\lass{Q}}{\lass{R}}\right)\in I}\fenceAss{} \strictfences \left\{\interAss{[\token{A}]}{\bar x}{\lass{P}}{\lass{Q}}{\lass{R}} \right\}
	}		

	\infer{
		\fenceAss{} \!\strictfences\! \left\{\interAss{[\token{A}]}{\bar x}{\lass{P}}{\lass{Q}}{\lass{R}} \right\}		
	}
	{
		\fenceAss{} \sepish (\lass{P} * \lass{R}) \slentails \m{false}
	}	
...
%	}	
	\infer{
		\left\{\interAss{[\token{A}]}{\bar x}{\lass{P}}{\lass{Q}}{\lass{R}} \right\} \weakenI{\fenceAss{}}
		\emptyset
	}
	{
	  \separate{\fenceAss{}}{\lass{P}}
...
%	[\proofRule{Expand/Contract}]
	{
		\left\{\interAss{[\token{A}]}{\bar x}{\lass{P}}{\lass{Q}}{\lass{R}} \right\} \;\approx^{\fenceAss{}}\;  \bigcup_{i \in I} \left\{\interAss{[\token{A}]}{\bar x}{\lass{P}}{ \lass{Q}}{\lass{R} * \lass{R}_i} \right\}	
	}
	{
		\fenceAss{} \sepish (\lass{P} * \lass{R}) \slentails \bigvee_{i \in I} \fenceAss{} \sepish \left(\lass{P} * \lass{R} * \lass{R}_i \right)
		&
...
%	[\proofRule{Disj}]
	...
	\infer{
	  \fenceAss{} \fences \left\{\interAss{[\token{A}]}{\bar x}{\lass{P}}{\lass{Q}}{\lass{R}}\right\}
	}
	{
	...
	\infer{
		\fenceAss{} \fences \left\{[\token{A}]\!\!:\exsts{\bar x} \action{\lass{P}}{\lass{Q}}{\lass{R}} \right\}	
	}
	{
		\separate{\fenceAss{}}{\lass{P}}
	}	
...
\begin{definition}[Reification]\label{def:reification}
  ...
  defined as follows, where \heapPart{l'} denotes the first projection
  of the pair $l'$:
  \[
