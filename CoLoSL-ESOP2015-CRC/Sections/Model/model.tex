...
and 
$r$, written $\extendsAM{\lmod}{s}{r}{\semI{I}}$, 
...
\semI[\lenv]{I}(\token{A}) &==
  ...
  \begin{array}{@{}l@{\ }|@{\ }r@{}}
    ...
    \begin{array}{@{}l@{}}
      ...
      %\for{l} l \leq p \land l \leq q => l = \unitL/|\null\\
      c = r \composeL r' \land \null\\
...
definition of worlds (Def.~\ref{def:worlds}) as $g\confines \lmod$, 
...
before definition~\ref{definition3}. The action $a_0 = ((\emptyset, \{\ell\,{:}\,0\},
...
Let us now focus on the third of these rules, which states that
...
%
	...
	\infer{
	  \stable{\lass{P} * \bigvarstar_{i\in J}\shared{\lass{Q}_i}{I_i}}
	}
	{
          \for{j\in J}
	  \stable{\lass{Q}_j, I_j, \lass{P}, \bigocon_{i\in J} \lass{Q}_i}
	}
...
%	
	...
	{
	  (\lass{P}_1 \septraction (\lass{Q} ** \lass{P}_1)) * \lass{P}_2 \slentails \lass{Q} * \m{true}
	}
\end{mathpar}
\hrule
%\vspace*{5pt}
\caption{Selected rules for stability checks.
 We assume that variables in
  $\bar x$ do not appear free in $\lass{P}$, $\lass{Q}$, and $\lass{R}$.}
\label{fig:stability-rules}
\end{figure*}

The first of these rules  is unfamiliar. Unlike fencing, stability
checking for assertions only has to be
checked against actions for which the environment may have the
capability. If the capability required by the action cannot exist
separately from those held by the assertion  (that is, those in $p *
r$) then the environment cannot possibly own
the capability to perform the action. Similarly, an action whose
precondition is incompatible with the assertion
$p*r$ cannot possibly fire, as stated by the   next rule.
%
%The current
...
%state).
%
The last rule checks that 
$\lass{Q}$ is preserved by the effect of an action. Again, there is a
crucial difference with the corresponding check for fencing:
in $p_2$,
the action may bring in some newly-shared state, hence the result $q *
\m{true}$;
but the \forgetRule rule allows us to immediately discard
it, if appropriate. 
