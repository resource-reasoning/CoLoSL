...
their static interference, which   limit how we  work
with a
...
The static interference relation 
 is defined when
the region  is created and    is fixed throughout the entire lifetime of
the region. However, inteference can become irrelevant due to resource
allocation, threads having access to parts of a region need  only
 know about the interference associated with those parts of the
region and , at the time  of creation, it is not necessarilly possible
to capture all future interactions associated with  a region. 
...
thread requires access to the $i$th and $(i+1)$th nodes. The threads
access overlapping shared resources (the two nodes) with interference 
relative to that overlapping resource. Also, consider a binary graph accessed by a concurrent
spanning tree algorithm. Disjoint regions with static
interference cannot simply be
used to reason about  data structures with intrinsic sharing which are
accessed concurrently. Finally, consider a concurrent set. ....
