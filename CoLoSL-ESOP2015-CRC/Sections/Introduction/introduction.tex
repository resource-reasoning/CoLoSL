...
their resource and interference expanding and contracting in
...
\pgcomment{What do we want to say about deny-guarantee? Does
  deny-guarantee have static regions with interference? I know CAP
  adapted interference lots, but I don't know if we swere the first to
  introduce it}
...
partial states. With LRG, it is possible to frame off both parts of
shared state and parts of the rely-guarantee relation, but, as noted
...
with subjective views to reason locally about overlapping shared
state.
...
Finally, we should mention Subjective Concurrent Separation Logic
(SCSL)~\cite{SCSLand new referenceesop14} of Navevski {\em et al.}, which has a different notion
...
possible to frame off partial concurroids. Unlike CAP,
concurroids have three parts: the joint part (j);  and the disjoint self  (s)
 and   other part (o), hence  their very different use of  the adjective `subjective'.
Although SCSL did  not influence \colosl, it does 
highlight  an interesting point regarding
resource transfer between regions: in CAP, communication between
regions is achieved  indirectly via  the local state; in LRG, communication
...
subjective views happens directly after the views have been merged. 
