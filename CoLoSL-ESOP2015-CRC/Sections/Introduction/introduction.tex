...
interaction. However, despite substantial progress, we believe that
none of these approaches are fully compositional. They are all based
on global shared memory often comprising static disjoint regions with
static interference on these regions, with each thread verified with
respect to these shared regions. The static nature of the regions
impedes compositionality, since the code of the thread often
manipulates just a small part of the region.  In contrast, we
introduce the program logic, \colosl, which is based on one global
shared state with each thread verified with respect to its partial
{\em subjective view} of this global state. The dynamic overlapping
nature of subjective views provides truly compositional reasoning,
which we demonstrate on a range of examples.
...
fine-grained concurrency: RGSep~\cite{.} and then deny-guarantee reasoning~\cite{}
...
thread and the environment can effect the region; CAP~\cite{.} and its
extensions~\cite{.} in addition abstract the regions.  This work has achieved substantial
success; see related work for more details. However, we believe that
none of these approaches are fully compositional.  We identify the
following three interconnected problems with shared regions and
their interference. 

First and foremost, the focus on static  disjoint shared regions is too coarse-grained.  
For instance, consider  a
linked list consisting of $n+1$ nodes accessed concurrently by $n$
threads (numbered $1$ to $n$) where the $i$th thread requires access
to the $i$th and $(i+1)$th  nodes. In this example, there is a natural
overlapping  of the resources accessed by the threads which cannot be
mimicked by the reasoning. Since shared regions must be disjoint, the
reasoning can only work with the entire linked list, even though  a thread is just working with a small part of the
list.
\pgcomment{Can we relate this linked list example to the set
  example??? This currently is not persuasive. People would say just
  use the whole list.???}  
Now consider  a binary graph accessed by a concurrent
spanning tree algorithm, as   an example of a
data structure with intrinsic sharing which is accessed
concurrently. Static disjoint shared regions cannot be used to reason
about such algorithms due to  the arbitrary sharing associated with
the subgraphs. 
\pgcomment{Now can we think of a specific third example to do with the
  natural sharing of two modules.} 
Crucial ingredients of compositionality is missing using disjoint
shared regions: one cannot give
small specifications to programs manipulating shared data; nor can  one
use the frame rule over shared state. 
