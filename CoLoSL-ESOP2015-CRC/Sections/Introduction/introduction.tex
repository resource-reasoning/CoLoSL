...
specifications for concurrent programs, which can impede
compositionality.
...
the environment may affect these parts, thus enabling small
specifications in the presence of concurrent behaviours.
...
ring, we are able to prove each thread in isolation, and the proved
specification only mentions two of its neighbours. Thanks to this
small specification, the proof can be largely reused when the
...
overlapping sub-graphs. We demonstrate that the flexible, overlapping
subjective views of \colosl are just what we need. Finally, our proof
of a concurrent set module implemented using a hand-over-hand
list-locking algorithm improves over that of CAP significantly.
...
work on separation logic~\cite{2001poplandjohn'ssurvey} demonstrated that such reasoning is not
