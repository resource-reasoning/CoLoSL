...
The algorithm %presented in \fig\ref{fig:concurrentInc},
...
Fortunately, we can do better and obtain a stronger small precondition.
...
variable \li{n} holding the current size of the ring, as does its
...
thread running \li{slave(x+$i$)} ($i>1$) gets $[\token{s}_{x{+}i}] *
...
For instance, let us now modify the third line of \li{ring(x)} by
...
render them thread-local. Instead, this behaviour can be simply
encoded 
%for a resource described by $P$ and associated with tokens
%$[\token{a}_1]$, \ldots , $[\token{a}_{n}]$ 
...
of \li{master(x)} has to account for the new
