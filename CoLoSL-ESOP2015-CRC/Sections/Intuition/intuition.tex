...
they need. This is not possible in existing frameworks. At first glance, the benefits of a small specification may not be obvious. However, as we shortly demonstrate small
specifications make proofs robust against changes to each thread's
environment, and provide more opportunities for \emph{proof reuse}.
% For
...
For instance, let us now modify the code of \li{ring(x)} in line~3 by forking an additional thread that dynamically grows the ring by spawning more slave threads (we leave the details to the accompanying technical report). In the general case of growing the ring of size $n$, we use an instance of the \extendRule principle as follows in order to add a new slave (at $x{+}n{+}1$) to the shared state. In doing so, we also define the new slave's associated interference and accompanying capability. \vspace{0pt}
%
\begin{align*}
	\cell{(x{+}n{+}1)}{v} \semimplies [\token{s}_{x{+}n{+}1}] * \shared{(\cell{x{+}n{+}1)}{v}}{\{s_{x{+}n{+}1}\}} \\[-20pt]
\end{align*}
%
With this modified code, our proof changes only minimally: the proof of \li{master(x)} has to be adapted slightly to account for the new interference on the value of $n$ since the environment can grow the ring and thus increment the value of $n$. However, since the code of \li{slave(x)} does not depend on $n$ (and thus $n$ is not observed in the subjective view of the precondition), its proof remains unchanged. 
