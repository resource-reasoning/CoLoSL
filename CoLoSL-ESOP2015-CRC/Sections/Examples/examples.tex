...
the unmarked state, the whole cell $\cell{x}{0,l,r}$ resides in the shared state. In the
marked state, the shared state only contains $\cell{x.m}{1}$: pointers to the left
and right subgraphs have been claimed by
the thread who marked the node since other threads need not access
...
that of marking $n$; which can be carried out without requiring any capabilities ($[\emptyset]$). 
...
\begin{figure}
    \hrule
    $\begin{array}{@{\hspace{10pt}} r @{\hspace*{2pt}} l}
			\graph{x}{\gamma} \eqdef & \shared{\G{x}{\gamma}}{I_\gamma} \hspace*{0.5cm} I_\gamma \eqdef \bigcup\limits_{n \in \gamma}I(n)\\
%	
			\G{x}{\gamma} \eqdef & (x = \li{null} \land \emp) \lor x \in \gamma \land \exsts{l, r} \gamma(x) = (l, r) \\
			& \land \left( \unmarked{x}{l}{r} \lor \marked{x}\right) \sepish \G{l}{\gamma} \sepish \G{r}{\gamma}\\
%
			...
			\marked{x} \eqdef  \cell{x}{1}\\
%
			I(n) \eqdef & \left\{ [\emptyset]: \exsts{l, r} \unmarked{n}{l}{r} \swap \marked{n}\right\}
		\end{array}
		$
    \hrule
...
%		\left(
%		\begin{array}{@{} l @{}}
			\li{b} = 0 \lor (\li{b} = 1 * \tr{\li{n}}{\gamma}) 
...
$x$, namely the left and right pointers
...
%% \]
%% %
In order to derive the local specification $\g{x}{\gamma}$ from the global specification $\G{x}{\gamma}$, it thus suffices to show $\shared{P}{I_{\gamma}} => Q$ as demonstrated below.
%
%\begin{align*}
%	&\shared{P}{I_{\gamma}}\!\!\!\!
...
\begin{align*}
	\shared{P}{I_{\gamma}} &
	\color{blue}\stackrel{(\copyRule)}{=>}\color{black}
	\underbrace{\shared{P}{I_{\gamma}} * \cdots * \shared{P}{I_{\gamma}}}_{|\gamma| \text{ times}}\\
	&\color{blue}\stackrel{(\forgetRule)}{=>}\color{black}
	\iterStar_{n \in \gamma} \left( \gamma(n) = (l, r) \land \shared{\unmarked{n}{l}{r} \lor \marked{n}}{I_{\gamma}}  \right)\\
	& \color{blue}\stackrel{(\shiftRule)}{=>}\color{black}
	\iterStar_{n \in \gamma} \left( \gamma(n) = (l, r) \land \shared{\unmarked{n}{l}{r} \lor \marked{n}}{I(n)}  \right)
...
\begin{lstlisting}
  ...
  b:= span(x) $\{$
    ...
    res:= $\langle$ CAS(x.m, 0, 1) $\rangle$;
    //<@\codecomment{$\left\{\begin{array}{@{} l @{}} \exsts{l, r} \gamma(\tx{x})= (l, r) \land\shared{\marked{\tx{x}}}{I(\tx{x})} *  \g{l}{\gamma} * \g{r}{\gamma} \\ * \left(\tx{res} = 0 \lor  \left(\begin{array}{l} \tx{res} = 1 * \cell{\tx{x}.l}{l} * \cell{\tx{x}.r}{r} \end{array}\right)\right)\end{array}\right\}$}@>
    //<@\codecomment{$\left\{\begin{array}{@{} l @{}} \tx{res} = 0 \!\lor\! \left(\!\tx{res} = 1 * \exsts{l, r} \gamma(\tx{x})= (l, r) \land\shared{\marked{\tx{x}}}{I(\tx{x})} \!*  \g{l}{\gamma} * \g{r}{\gamma} * \cell{\tx{x}.l}{l} * \cell{\tx{x}.r}{r} \!\right)\end{array}\right\}$}@>
    if (res) then $\{$ 
      //<@\codecomment{$\left\{\begin{array}{@{} l @{}} \tx{res} = 1 * \exsts{l, r} \gamma(\tx{x})= (l, r) \land \shared{\marked{\tx{x}}}{I(\tx{x})} * \g{l}{\gamma} * \g{r}{\gamma} * \cell{\tx{x}.l}{l} * \cell{\tx{x}.r}{r}    \end{array} \right\}$}@>
	 $\begin{array}{l @{\hspace{10pt}} || @{\hspace{10pt}} l} \color{blue} //\{\g{l}{\gamma}\} & \color{blue} //\{\g{r}{\gamma}\} \\ \tx{b1:= span([x.l])} & \tx{b2:= span([x.r])} \\ \color{blue} //\{\tx{b1} = 0 \lor (\tx{b1} = 1 * \tr{l}{\gamma}) \} & \color{blue} //\{b2 = 0 \lor (b2 = 1 * \tr{r}{\gamma})\} \end{array}$
      //<@\codecomment{$\left\{\begin{array}{@{} l @{} } \tx{res} = 1 * \exsts{l, r} \gamma(\tx{x})= (l, r) \land \shared{\marked{\tx{x}}}{I(\tx{x})} * \cell{\tx{x}.l}{l} * \cell{\tx{x}.r}{r}  \\  * \left( \tx{b1} = 0 \lor (\tx{b1} = 1 *  \tr{l}{\gamma}) \right) * \left( \tx{b2} = 0 \lor (\tx{b2} = 1 * \tr{r}{\gamma}) \right)  \end{array}\right\}$}@>
      if (!b1) then 
        $\text{[}$x.l$\text{]}$:= null
      if (!b2) then 
        $\text{[}$x.r$\text{]}$:= null
      //<@\codecomment{$\left\{\begin{array}{@{} l @{}}  \tx{res} = 1 * \exsts{l, r} \gamma(\tx{x})= (l, r) \land\shared{\marked{\tx{x}}}{I(\tx{x})} \\ * \exsts{l' \in \{l, \tx{null}\}} \cell{\tx{x}.l}{l'} * \tr{l'}{\gamma} * \exsts{r' \in \{r, \tx{null}\}} \cell{x.r}{r'} * \tr{r'}{\gamma}  \end{array}\right\}$}@>
      //<@\codecomment{$\left\{\begin{array}{@{} l @{}} \tx{res} = 1 *  \tr{\tx{x}}{\gamma}    \end{array}\right\}$}@>
    $\}$    
    //<@\codecomment{$\left\{ \begin{array}{@{} l @{}} \tx{res} = 0  \lor (\tx{res} = 1  * \tr{\tx{x}}{\gamma})  \end{array} \right\}$}@>
    return res
  $\}$
  //<@\codecomment{$\left\{ \begin{array}{@{} l @{}} \tx{b} = 0  \lor (\tx{b} = 1  * \tr{\tx{x}}{\gamma}) \end{array} \right\}$}@>
\end{lstlisting}
\hrule\vspace*{-6pt}
\caption{Concurrent Spanning Tree Implementation}
\label{fig:conSpanningTree}
...
This predicate is unnecessarily complicated. It is rather counter-intuitive to account for the capabilities pertaining to addresses not in the domain of the list. Moreover, it is overly verbose to consider all combinations of values and successor addresses. Lastly, each thread is required to ``know'' of the resources associated with all nodes in the list and account for their associated interference. 

%
We proceed with the \colosl\ proof of the set module. Recall from \S\ref{sec:colosl} that \colosl\ is parametric in the separation algebra of capabilities. We thus instantiate it with a heap-like capability separation algebra that is \emph{stateful} and demonstrate that this allows for a more concise proof.  
...
%
We can reason about the \li{remove} operation in a similar fashion. The dynamic extension afforded by the \extendRule\ principle allows us to generate new capabilities only when needed and thus gives way to a conciser proof. 
%
...
\newpage
TaDA~\cite{tada} took the first steps towards addressing the shortcomings of the CAP approach. In particular, since TaDA is parametric in the separation algebra of its capabilities (called guards), one can choose a more suitable algebra to encode the desired behaviour of capabilities. For instance, the extension of the set with new values and capabilities can be captured as an additional axiom of the set module specification as follows.\vspace{-7pt}
%
\begin{align}
	\textsf{set}(x, vs) <=> \textsf{set}(x, vs\uplus\{v\}) * [\text{U}(v)]  \tag{\textsc{Ext.}} \label{extend-axiom}\\[-20pt]\nonumber 
\end{align}
%
This axiom states that given an abstract set at address $x$ with values $vs$, the set may be extended with value $v$ together with the accompanying update capability $[\text{U}(v)]$. The module implementer must then demonstrate that the (\ref{extend-axiom}) axiom is indeed satisfied by the implementation. 

While TaDA's approach is much cleaner than that of CAP, it nevertheless requires the foresight of axiomatising all potential future extensions of the region. On the other hand, as well as being parametric in its capability separation algebra, \colosl allows the dynamic extension of the shared state on the go without the need for prior axiomatisation.
