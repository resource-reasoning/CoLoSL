...
The second and last kind of interference is the \emph{update} of the global state according to actions in the global action model whose capability is ``owned by the environment'', \textit{i.e.}, nowhere to be found in the current local and shared states.
%
\[	
  \updateR \eqdef
  \left\{
...
%
\[
  \rely \eqdef  \left(\updateR \cup \extendR \right)^{\text{*}}
\]
%
\end{definition}
...
%
\begin{align*}
	& S^{0} \eqdef \extendR \cup \updateR  \cup \left\{(w, w) \mid w \in \Worlds \right\} \\
...
\end{align}
%
We are then required to show $w', \lenv |= P$. From (\ref{stable:Ass1}) there are then three cases to consider:
%(\ref{stable:Ass})
\begin{enumerate}
...
The \emph{combination} of an assertion $P \eqdef \exsts{\bar{x}}P'$ describing the current state, and an assertion $Q \eqdef \exsts{\bar{y}}Q'$ describing an action precondition, $\combine{.}{.}: \Assertions \times \Assertions \rightarrow \LAssertions$, is defined as follows. We assume that $P$ and $Q$ are in the prenex normal form with no bound variables in $P'$ and $Q'$. 
%
\[
\begin{array}{l l}
...
\hrule
%\vspace*{5pt}
\caption{Stability judgements where $P, Q, Q_1, Q_2, R \in \FAssertions$; $\lass{P}, \lass{q}_1, \lass{q}_2 \in \LAssertions$ and $\dot \in \{\lor, *, **\}$.}
...
\[
\guarantee \eqdef  \left( \updateG \cup \extendG \right)^{\text{*}}
\]
\end{definition}
...
% except for the overlapping conjunction ($**$) and septraction ($\septraction$) connectives. We thus provide a number of elimination rules for $**$ in \fig\todo; $\septraction$ can be eliminated as described by Calcagno et al. in~\cite{vv07msc}.
%
\begin{figure}
\hrule\vspace{5pt}
\begin{mathpar}
...
\end{mathpar}
\hrule
\caption{Confinement/local fencing judgements with $P, Q \in \Assertions$; $\lass{P}, \lass{Q}, \lass{R}, \lass{f} \in \LAssertions$.}
...
The capability assertion $\capAss{}$ with logical variables $\bar{x}$ is \emph{fresh}, written $\fresh{\bar{x}, \capAss{}}$, iff for all $\lenv \in \LEnv$ and $K \in \pset{\Caps}$ where $\wdCapSet{K}$, there exists $\ca{} \in \Caps$ such that
%The capability assertion $\capAss{}$ with logical variables $\bar{x}$ is \emph{fresh}, written $\fresh{\bar{x}, \capAss{}}$, iff for all $\lenv \in \LEnv$ and $K \in \ADoms$, there exists $\ca{} \in \Caps$ such that
%
\[
