...
To reason about this program, following Hobor et al. we use two representations of graphs~\cite{ramification}. The
...
mathematical graph $\gamma$. The predicate defined in \fig~\ref{fig:globalCST} uses the
...
%
%
The elements of the list are represented as a singly-linked list with no duplicat elements; the list starts at address $x$ with value $v_1$ and points to the next element at address $y$ with value $v_2$ and so forth. In the remainder of this section, we write $\node{x}{v}{y}$ to denote a node at address $x$, with value $v$ and successor $y$. 
...
\subsection{Concurrent Database}
In Appendix~\ref{sec:mutual-exclusion} we generalise the mutual exclusion ring of \S\ref{sec:intuition} to $n$ threads and demonstrate how this can be used to synchronise access to an $n$-threaded concurrent database. 