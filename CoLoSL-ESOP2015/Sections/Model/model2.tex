...
shared state $g$ (i.e.\ $g = s\composeL r$ for some context
$r$), such that $s$ satisfies $P$ in the standard separation logic
sense, and $I$ and $\lmod$ \emph{agree} given the decomposition $s$,
$r$, in the sense that $I$ accounts for all possible interference on $s$, which we write
as $\extendsAM{\lmod}{s}{r}{\semI{I}}$ (we omit the definition of $\downarrow$ for lack
...
must be invariant under all actions of $I$. In Appendix~\ref{sec:assertions-continued} we show that it is always possible to weaken an assertion into a local assertion. This fencing condition is
