...
We write $l$ to range over either arbitrary logical states or those
...
(resp.\ heap) where only \li{x} (resp.\ address $E_1$) is allocated
...
that do not. For lack of space, we omit these transformations from the
paper; they may be found in \S XXX.
...
can be expressed in a fragment extending the usual \emph{symbolic
  heaps} used in automatic tools based on separation
logic~\cite{smallfoot,smallfoot-rg,spaceinvader,abductor}, without
inductive predicates but including the $**$ connective (this
corresponds to replacing $p$ with $A$ in the grammar of assertions),
the separation logic entailments that need to be discharged once the
rules of this section have been applied can be verified
automatically. Indeed, for this fragment, we can extend the
$--o$-elimination procedure of Calcagno et al. in~\cite{vv07msc} to
deal with $**$ as well. We leave the details of this adaptation to \S
XXX.
...
In the logic, the shared state is extended by the resources in $\lass{P}$
under the interference $I$ via the \extendRule principle, which
requires that $\lass{P}\containI I$. \fig~\ref{fig:local-fencing-rules}
presents a set of rules to establish $\lass{P}\containI I$. As expressed by
the first rule of \fig~\ref{fig:local-fencing-rules}, $\lass{P}\containI I$
holds if there is a weaker assertion $\lass{F}$ that acts as a \emph{local
  fence} for $I$, denoted by $\lass{F}\strictfences I$. This fencing
condition is checked for each action in $I$ (as expressed by the
second rule). For each action $\lass{P}\swap \lass{Q}$, the three remaining rules of
the figure may apply. In the first of these rules, the action cannot
possibly fire, because its precondition is incompatible with $\lass{F}$: no
state satisfying $\lass{F}$ may be extended such that a sub-part satisfies
$\lass{P}$. The second of these rules allows one to trim neutral parts $\lass{R}$ of
an action $\lass{P}\swap \lass{Q}$: if $\lass{R}$ appears both in $\lass{P}$ and $\lass{Q}$, and $\lass{R}$ is
exact, then the part of the state denoted by $\lass{R}$ is uniquely
determined and left unchanged by the action. The last rule finally
reduces local fencing to entailment checking, provided the fence $\lass{F}$
can be expressed as a disjunction of precise formulas. The first
premise states that $\lass{F}$ is invariant under the action $\lass{P}\swap \lass{Q}$, in a
similar fashion as i.e.\ RGSep encodes stability checks as separation
logic entailments. The third premise checks the confinement condition:
if a sub-part of an extension of a state in $\lass{F}_i$ satisfies $\lass{P}$
($l_1\composeL l_2|= \lass{F}_i$ and $l_2\composeL l_3 |= \lass{P}$), then that it
is also a sub-part of that state ($l_1\composeL l_2\composeL
l_3|=\lass{F}_i$, hence by precision of $\lass{F}_i$, $l_2\composeL l_3\leq
l_1\composeL l_2\composeL l_3 = l_1\composeL l_2$).
...
\begin{mathpar}
	...
	{
          \for{\left([\token{A}]: \exsts{\bar x}\lass{P} \swap \lass{Q}\right)\in I}\fenceAss{} \strictfences \left\{[\token{A}]:\exsts{\bar x} \lass{P} \swap \lass{Q} \right\}
	}		
...
%	[\proofRule{Expand/Contract}]
	{
		\left\{[\token{A}]: \exsts{\bar x}\lass{P} \swap \lass{Q} \right\} \;\approx^{\fenceAss{}}\;  \bigcup_{i \in I} \left\{[\token{A}]: \exsts{\bar x}\lass{P} * \lass{R}_i \swap \lass{Q} * \lass{R}_i \right\}	
	}
	{
...
another $\lass{F}$, which if it \emph{fences} for $I_1\cup I_2$, can be used
to reduce a shifting judgement $I \cup I_1 \weakenI{\fenceAss{}} I
\cup I_2$ to $I_1 \weakenI{\fenceAss{}} I_2$. To check whether an
assertion $\lass{F}$ fences an interference assertion $I$, written $\lass{F}\fences
...
\item
  When $\lass{P}$ and $\lass{Q}$ do not intersect in an action $\lass{P}\swap \lass{Q}$, for
  ...
  $\lass{F}$. Because the action is allowed to act partly outside of $\lass{F}$, the
  state that is mutated has to be $\lass{F} ** \lass{P}$, unlike local fencing where
  $\lass{P}$ would be contained in $\lass{F}$. The reason why $\lass{P}$ and $\lass{Q}$ must not
  interesect is precisely that we consider action on $\lass{F} ** \lass{P}$ and not
  just $\lass{F}$; if $\lass{P}$ and $\lass{Q}$ have a non-empty intersection $l$ then it might
  be that $l$ is not part of $\lass{F}$ and is added together with $\lass{Q}$ by $*$
  ...
  since it was not part of $\lass{F}$, hence we could end up with an
  assertion $\shared{\lass{F}}{I}$ that does not satisfy the action closure
  property anymore.
\end{enumerate}
...
to a particular assertion $\lass{F}$: if the effect of the action is entirely
outside of $\lass{F}$, or if it is impossible to apply the action to $\lass{F}$. The
next rule is an equivalence that use the knowledge of the possible
states $\lass{F}$ of the shared region to rewrite an action into an
equivalent one. More precisely, if whenever the precondition $\lass{P}$ of
the action is compatible with $\lass{F}$ then one of the $\lass{R}_i$'s is also
true, then it is safe to add $\lass{R}_i$ as a neutral part of the action
(the fact that $\lass{R}_i$ is exact guarantees that no piece of state in
$\lass{R}_i$ is mutated by the action) without changing the set of states in
...
of the form $\exsts{\bar x}\lass{P} * \bigstar_{i\in I}\shared{\lass{Q}_i}{I_i}$ is
stable, where $\lass{P}$, $\lass{Q}_i$ and assertions in $I_i$ are local for all
...
eliminated. Then, checking the stability of $\lass{P} * \bigstar_{i\in
  I}\shared{\lass{Q}_i}{I_i}$ boils down to establishing, for each subjective
view $\shared{\lass{Q}_i}{I_i}$, the four-place $\mathsf{Stable}$ predicate
which, in this case, means that $\lass{Q}_i$ is stable under the interference
assertion $I_i$, a local context $\lass{P}$, and a shared context made of the
$**$-combination of all the subjective assertions (including $\lass{Q}_i$).
...
the last rule, which checks that a particular subjective assertion $\lass{Q}$
...
%
	...
	\infer{
	  \stable{\lass{P} * \bigstar_{i\in I}\shared{\lass{Q}_i}{I_i}}
	}
	{
          \for{i\in I}
	  \stable{\lass{Q}_i, I_i, \lass{P}, \bigocon_{j\in I} \lass{Q}_j}
	}
...
%	
	...
	}
	{
          ...
          \stable{\lass{Q},\{\interAss{[\token{A}]}{\bar{x}}{\lass{P_2}}{\lass{P}_2}\},\lass{P},\lass{R}}
	}	
