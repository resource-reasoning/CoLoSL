...
states (\textit{e.g.} stacks and heaps); the other represents
...
deny-guarantee~\cite{dg} or, \textit{e.g.}, CAP~\cite{cap} for further
...
\emph{confined} to $g$ (written $g\containI\lmod$), \textit{i.e.}
...
The assertions of \colosl are elements of $\Assertions$ described by the grammar below, where $x \in \set{LVar}$, $\li{x} \in \set{PVar}$, and $\token{a} \in \set{Token}$. We write $[A]$ as a shorthand for $\oast_{\token{a} \in A}[\token{a}]$.
%
\begin{align*}	 
...
%
  I &::= \emptyset \mid \{\interAss{[A]}{\bar{y}}{P}{Q}\} \cup I
\end{align*}
\end{definition}
%
...
\semI[\lenv]{I}(\token{A}) &==
  ...
  \begin{array}{@{}l@{\ }|@{\ }r@{}}
    ...
    \begin{array}{@{}l@{}}
      \interAss{A}{\bar{y}}{P}{Q} \in I /|  \exsts{\bar{v}, \lmod} \\
...
they are conserved by the action (\textit{i.e.}\ are present in $q$).
%
...
from the succive application of actions in $\lmod'$ themselves confine
all actions in $\lmod'$.

In the logic, the shared state is extended by resource in $P$ under
...
$P,Q\in\LAssertions$, \textit{i.e.} the action assertions do not
...
the action $P\swap Q$, in a similar fashion as \textit{i.e.} RGSep
...
\begin{mathpar}
	...
	{
          \for{\capAss{}: \lass{P} \swap \lass{Q}\in I}\fenceAss{} \strictfences \left\{\capAss{}: \lass{P} \swap \lass{Q} \right\}
	}		
...
\begin{mathpar}
	...
	{
          \for{\capAss{}: \lass{P} \swap \lass{Q}\in I}\fenceAss{} \fences \left\{\capAss{}: \lass{P} \swap \lass{Q} \right\}
	}		
...
%	[\proofRule{Hide}]
	{
		\left\{ \capAss{}: \lass{P'}  \swap \lass{Q'} \right\} \weakenI{P}
		\emptyset
	}{	
		\exact{\lass{R}}
		&
...
%	[\proofRule{Disj}]
	{
	  \bigcup\limits_{i \in I,j\in J}\!\! \left\{\capAss{}\!\!:\!\exists\bar{y}. P_i \!\swap\! Q_j \right\}
	  \mathrel{\approx^{\m{true}}}
	  \left\{\!\capAss{}\!\!: \exsts{\bar{y}}
