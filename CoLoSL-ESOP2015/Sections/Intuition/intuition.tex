...
separation logic~\cite{csl-tcs}. We will explain them as needed during
the proof of our example.
...
\begin{align*}
  (P => Q)
  &=>
  \shared{P}{I} => \shared{P}{Q}
  &\mathllap{(\weakenRule)}&
  &
  \shared{P}{I} &
  => \shared{P}{I} * \shared P I \tag{\copyRule}
  \\
...
interferences from the environment (as prescribed in $I$).
 ...
 Subjective views can always be duplicated using the \copyRule principle:\vspace*{-17pt}
%
...
%Lines starting with $\color{blue}{//}$ contain formulas that describe the local state and the subjective shared state at the relevant program point.
}
...
As a first try at simplifying the precondition, consider the following implication using the \forgetRule and \shiftRule principles given in the introduction\vspace*{-5pt}
%
\begin{align*}
 	\shared{\li{x}|-> 0 * \li{y}|->0 * \li{z}|-> 0}{I} *[\token a_{\li y}]
...
Finally, we stabilise the resulting subjective view such that it is invariant under all possible actions by the environment.
...
Consider the action of $[\token a_{\li x}]$ in $I$ and the initial state with value $0$ in all the cells. This action can be replaced by \vspace*{-5pt}
%
\[
	[\token a_{\li x}] : 
...
\]
%
This is possible  because, as the programmer knows, whenever \li{x} and \li{z} have the same value then \li{y} also has the same value which, under these conditions, is not changed by the actions in $I$. 
This amended action reflects stronger knowledge about when \li{x} can be incremented and how its value is related to \li{y} and \li{z}.
...
As we justify in \S~\ref{sec:todo}, by using the judgement $I\weakenI{\cell{\li{x}}{0} * \cell{\li{y}}{0} * \cell{\li{z}}{0}} I_{\li y}'$, we can use the \shiftRule principle to replace $I_{\li y}$ by $I'_{\li y}$. 
%%
%\[
%I\weakenIb{\cell{\li{x}}{0} * \cell{\li{y}}{0} * \cell{\li{z}}{0}} I_{\li y}'
%\]
%%
Using \forgetRule, it is now safe to lose the \li{z} assertion to obtain the subjective view  $\shared{\li{x}|-> 0 * \li{y}|->0 }{I'_{\li y}}$, as the new action of $[\token a_{\li x}]$ in $I'_{\li y}$ retains enough information about how \li{x}, \li{y} and \li{z} are related.
Since the action of $[\token a_{\li z}]$ only affects the \li{z} cell, which has been forgotten, leaving  the cells \li{x} and \li{y} unaltered, we can use the \shiftRule\ principle again to change the interference relation to $I_{\li y} = I'_{\li y} \backslash \token a_{\li z}$. The interference relation is now as simple as it can get, whilst retaining enough information about the connection between \li{x}, \li{y} and \li{z}. Finally, we stabilise the subjective view with respect to $I_{\li y}$ and obtain our final precondition of $\mathbb{P}_{\li y}$.
%, which retains much more information about the values of \li{x} and \li{y}. %

There is one more subtlety to mention about this precondition. The thread has the capability $[\token a_{\li y}]$ to perform its action which only requires the resources described by the subjective view. However, the action of $[\token a_{\li x}]$ depends on \li{z}, which is no longer a part of the subjective view of the thread. Since another thread in the environment may own the $[\token{a_{\li x}}]$ capability, it may perform its action whenever its subjective view is {\em compatible} with the pre-condition of the action.
...
This concludes our \colosl proof of $\mathbb{INC}$. Our expansion and contraction of subjective
