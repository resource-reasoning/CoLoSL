...
about why their implementations are correct. 

Separation logic reasoning~\cite{seppaperand concurrent sep logic} has
...
\pgcomment{Next comment is important, but it does not fit wonderfully
  here. Maybe in conclusions?} 

 Finally, while in some examples we know have complete knowledge of the
global shared state, this is not necessarily the case  when proving code from a library.
