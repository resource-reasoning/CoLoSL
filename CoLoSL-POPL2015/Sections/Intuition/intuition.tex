...
of the thread. 
Actions performed by the environment can happen any
time that the environment's subjective view is {\em compatible} with
the thread's subjective view.
\azaleacomment{This is very misleading. The environment's view IS AND ALWAYS SHOULD BE compatible with the subjective view of the current thread.}  When that is the case, the piece of the
