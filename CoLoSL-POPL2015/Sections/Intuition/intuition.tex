...
showcased by the proof above. First, this example merely mutates and
extends the shared state, while in general actions are also allowed to
temporarily or permanently exchange resource between the local and
shared state of a thread. More precisely, an action $P\swap Q$ may
\begin{enumerate}
\item
  \emph{modify} resources: $P$ and $Q$ contain the same amount of
  resource with different values, \textit{e.g.}
  $\token{a}_{\tx{x}} \in I$;
\item \emph{remove} resources from the shared state and move them into
...
an explicit \emph{unsharing} mechanism in order to claim shared
resources and render them thread-private. Instead, this behaviour can
...
view, then weaken the resulting assertion to remove it from the
