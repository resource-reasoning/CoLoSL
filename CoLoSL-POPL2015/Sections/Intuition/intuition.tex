...
\]
can increment  the cell given by \li{y} under the condition that the
value of the  cell
...
&=>  \shared{\exists v, v'.  (\li{x}|-> v * \li{y}|->v' ) \wedge v\geq v'}{I\backslash \token a_{\li z}} *[\token a_{\li y}]
 & \textsc{(Weaken)}\\
\end{align*}
The thread $\mathbb{P}_{\li y}$ does not modify \li{z} so we are able to  forget the
cell referenced by \li{z}. Since there is now no cell given by \li{z}, the
...
resource given by \li{z}, which is not now part of the subjective view
