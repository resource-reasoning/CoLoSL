...
views and the ability to manipulate interference assertions using
semantic insights about the possible evolutions of the shared state
gained at the time where we can see the whole shared state enables the
specification and verification of each thread to be confined to just
the resources they need, a feat not possible using previous work.

\paragraph{\colosl Actions and Unsharing}
As discussed above, given a subjective view $\shared P I$, the interference assertion $I$ contains actions of the form $Q \swap R$ that state how the shared resources described by $P$ may be manipulated. The actions of $I$ capture three patterns of behaviour: 
\begin{enumerate}
...
%
%
We demonstrate the latter two behaviours through our example in \S\ref{subsec:CSTExample}. These three behaviours are not mutually exclusive and an action may exhibit any combination of them. 

Unlike~\cite{cap-ecoop10} and as with~\cite{icap}, we do not provide an explicit \emph{unsharing} mechanism in order to claim shared resources and render them thread-private. Instead, this behaviour can be simply encoded as an additional action with the second behaviour described above. 
