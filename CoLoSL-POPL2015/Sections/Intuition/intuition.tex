...
\begin{align*}
  I &\eqdef \left\{
  \begin{array}{@{}l@{\,}l@{}l@{}}
    \token a_x: & \exsts{v} & \cell{x}{v} * \cell{z}{v} \swap\\
    &&\quad \cell{x}{v+1} * \cell{z}{v}\\
...
techniques from concurrency reasoning, more specifically as inspired
by concurrent abstract predicates~\cite{cap-ecoop10} (CAP). In
particular, commands in angle brackets represent \emph{atomic
  sections}. Since all such sections are operationally mutually
exclusive, it is okay for the program to manipulate contents of the
subjective region when executing an atomic command. Logically, this
translates into the following (standard) proof rule
\[
\infrule{Atomic}
        {\hoare p {\mathbb P} q\\
...
At the very beginning of the execution of $\mathbb{INC}$, the state is
purely local and contains the three variables $x$, $y$, and $z$, which
allows $\mathbb P$ to initialise them to 0 without
syncronisation. Since the purpose of the rest of the program is to
share these variables between $\mathbb P_x$, $\mathbb P_y$, and
$\mathbb P_z$, a logical shared region is created, ruled by
interferences $I$ and the three capabilities $\token a_x$, $\token
a_y$, and $\token a_z$. The capabilities are automatically added to
the current local state upon region creation using the following
principle:
\begin{align}
  \label{eq:extend}
  P \containI I
...
done, as is standard in the concurrent separation logic family, using
the following concurrency rule:
\[
\infrule{Parallel}
...
\emph{sepish}~\cite{gareth-js12}) over subjective states (contrast
with most existing formalisms, which would use \emph{classical
  conjunction} instead).
\begin{align}
  \label{eq:merge}
  \shared{P}{I} * \shared{Q}{I} &<=> \shared{P \sepish Q}{I} \tag{\textsc{Merge}}
\end{align}
On local states, $*$ behaves as \emph{disjoint union}, as is standard.

In this instance, we use two successive applications of the rule and,
accordingly, we need to split the current symbolic state three ways
using $*$ such that each sub-formula is a valid precondition of the
corresponding thread.  We proceed by assigning the capabilities
$\token a_x$, $\token a_y$ and $\token a_z$ to $\mathbb P_x$, $\mathbb
P_y$ and $\mathbb P_z$, respectively. The subjective shared state is
split using the fact that, $P => P ** P$ for all $P$, and the
following derivation, where the steps involving \eqref{eq:shift} and
\eqref{eq:forget} will be explained in the next two sections. We let
$P_0 == x|-> 0 * y|->0 * z|-> 0 $. We delay the definitions of $I_x'$,
$I_y'$ and $I_z'$ until \S\ref{subsec:shift}.
\begin{align*}
  ...
  =>\null&
  \begin{array}[t]{L}
    (\shared{x|-> 0 * z|-> 0}{I_x'} *\token a_x)*\\
    (\shared{x|-> 0 * y|-> 0}{I_y'} *\token a_y)*
    \mathrlap{(\shared{x|-> 0 * z|-> 0}{I_z'} *\token a_z)}
  \end{array}
  &\eqref{eq:forget} \\
  =>\null&
  \begin{array}[t]{L}
  ...
  =>\null&
  \begin{array}[t]{L}
    (\shared{\cell{x}{0} * \cell{z}{0}}{I_x} * \token a_x) *\\
...
Similarly, $*$-conjoining the postconditions of $\mathbb P_x$,
$\mathbb P_y$ and $\mathbb P_z$ yields the overall postcondition of
$\mathbb P$ by \eqref{eq:merge}, except for the interference relation
which is $I_x\cup I_y\cup I_z$.  This uses the fact that $|/$
distributes over $**$, and, \textit{e.g.},
\[
\begin{array}{RL}
&(\cell{x}{10} * \cell{z}{10}) ** (\cell{x}{10} * \cell{y}{10})\\
<=>&
\cell{x}{10} * \cell{y}{10} * \cell{z}{10}
\end{array}
\]
Finally, the combined knowledge of all threads to get rid of spurious
disjunctions (such as $x|-> 11 * y|->10$, when in fact we know from
the postcondition of $\mathbb P_x$ that $x|->10$).
...
$I$ afterwards (see above). This is all accomplished by the
...
\begin{align*}
 I_x' &==
 I_x \cup \{\token a_y: \exsts{v}\cell{x}{v+1} * \cell{y}{v} \swap
 \cell{x}{v+1} * \cell{y}{v+1} \}\\
 I_y' &==
 I_y \cup \{\token a_z: \exsts{v}\cell{y}{v+1} * \cell{z}{v} \swap
 \cell{y}{v+1} * \cell{z}{v+1} \}\\
 I_z' &==
 I_z \cup \{\token a_x: \exsts{v}\cell{z}{v+1} * \cell{x}{v} \swap
 \cell{z}{v+1} * \cell{x}{v+1} \}
\end{align*}
...
after any performing any number of actions from $I_y'$). Thus, it is
semantically equivalent to drop this action, and similarly for $I_x'$
and $I_y'$:
\[
...
derivation of \S\ref{subsec:merge}.  In this section, we have
informally justified manipulations of the interference relation by
semantic arguments. However, \S\ref{sec:shiftax} will show how the
reasoning involved can be reduced to simple logical implications
between separation logic formulas that do not mention the shared
state.
...
We are now left with justifying the third step of the derivation of
\S\ref{subsec:merge}.
...
\end{align}
It is straightforward to finish the proof of \S\ref{subsec:merge}, and
thus of the program of \fig\ref{fig:concurrentInc} using this fact.

Note that the right-hand-side is not necessarily stable with respect
to $I$. This is why we first made $I$ more explicit
using~\eqref{eq:shift} in the derivation of \S\ref{subsec:merge}. Had
we used~\eqref{eq:forget} at the second step instead, we would have gotten
\[
\begin{array}[t]{L}
  (\shared{x|-> 0 * z|-> 0}{I} *\token a_x)*\\
  (\shared{x|-> 0 * y|-> 0}{I} *\token a_y)*
  (\shared{x|-> 0 * z|-> 0}{I} *\token a_z)
\end{array}
\]
where we no longer know the value of $y$ in relation to $x$ and $z$ in
the first conjunct $\shared{x|-> 0 * z|-> 0}{I} *\token a_x$, for
...
corresponding to the first conjunct would have to be
\[
