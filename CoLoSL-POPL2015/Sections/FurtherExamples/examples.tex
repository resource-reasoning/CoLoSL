...
\begin{array}{r @{\hspace*{2pt}} l}
	\graph{x}{\gamma} \eqdef & \markT{x}{\rootEdge} * \shared{\G{x}{\gamma}}{\bigcup\limits_{n \in \gamma}I(n)}\\
%	
...
\begin{array}{r @{\hspace*{2pt}} l}
	...
	\tr{x}{\gamma} \eqdef & (x = \nil \land \emp) \lor x \in \gamma \land \exsts{l, r} \gamma(x) = (l, r) \land\\
	& \exsts{l' \in \{l, \nil\}} \exsts{r' \in \{r, \nil\}}\\
	&  \shared{\marked{x}}{I(x)} * \markT{l}{x.l} * \cell{x.l}{l'} * \tr{l'}{\gamma} * \\
...
\]
%
\azaleacomment{explain the $\g{x}{\gamma}$ and $\tr{x}{\gamma}$ predicates. }
...
\]
%
It thus suffices to show $\shared{P}{\bigcup\limits_{n \in \gamma} I(n)} \semimplies Q$.
%
\[
\begin{array}{@{} c @{} l @{}}
	&\shared{P}{\bigcup\limits_{n \in \gamma} I(n)}  \\
	
	\stackrel{(\textsc{Copy})}{\implies} &
	\underbrace{\shared{P}{\bigcup\limits_{n \in \gamma} I(n)} * \cdots * \shared{P}{\bigcup\limits_{n \in \gamma} I(n)} }_{k \text{ times}}\\
	
	
	
	\stackrel{(\textsc{Forget})}{\implies} &
	\iterStar_{n \in \gamma} \left( \gamma(n) = (l, r) \land \shared{\unmarked{n}{l}{r} \lor \marked{n}}{\bigcup\limits_{n \in \gamma} I(n)}  \right)\\
...
\begin{lstlisting}
 ...
 b:= span(x) {
   ...
   //$\comment\{\texttt{res} \land \markT{x}{\rootEdge} *  \tr{x}{\gamma}\}$      
   }		
