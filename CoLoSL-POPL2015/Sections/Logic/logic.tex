...
\[
\begin{array}{r@{}l}
	P, Q  ::=\null& \emp \mid \heapAss{} \mid \capAss{} \mid P * Q \mid P \ewand Q \mid P \sepish Q \mid P \intersect Q \mid \\
	& \shared{P}{I} \mid  \exsts{x} P \mid \m{false} \mid P \Rightarrow Q \\
	I ::=\null& \interAss{\capAss{}}{\vec{y}}{P}{Q} \mid I_1 \cup I_2
\end{array}
\]
%
In $\Assertions$, $P \ewand Q$ denotes the existential magic wand of separation logic~\cite{todo} while $P \sepish Q$ denotes the overlapping conjunction or ``sepish''.
The $\shared{P}{I}$ assertion is \emph{subjective} and describes only parts of the shared state that satisfy $P$. The overall (global) shared state can be expressed as $P * \m{true}$ where $\m{true}$ represents all states \emph{compatible} with $P$. 
For instance, the $S_X$ predicate in \fig\ref{fig:concurrentIncSubjectiveSpec} describes parts of the shared state that contain the $x$ and $z$ variables, while the $G$ predicate of \fig~\ref{fig:concurrentIncCoLoSLSpec} defines parts of the shared state that contain the $z$ variable as well as $x$ and $y$.
 Both assertions are subjective views of the same underlying shared state and are compatible with one another; the former however yields a weaker and more localised description.
...
%	\right\}\\
	...
	\end{array} 
	\right\}\\
	\intermediateSem{P \!\intersect\! Q} 
	&  \eqdef
	\left\{ 
	\begin{array}{@{}l@{\ }|@{\ }l@{}}
		(pq, s, \amod{}, \amod{L}) & 
		\begin{array}{@{}l@{}}
				 (p \composeL pq, s, \amod{}, \amod{L}) \in \intermediateSem{P}\land \\
				 (pq \composeL q , s, \amod{}, \amod{L}) \in \intermediateSem{Q} 
		\end{array}
	\end{array} 
	\right\}\\
	...
	\begin{array}{@{}l@{\ }|@{\ }l@{}}
		...
		\begin{array}{@{}l@{}}
			...
			\null\land r \in \LStates \\
			\null\land \extendsAM{\amod{}, \amod{L}}{l}{r}{\semI{I} }
		\end{array}
	\end{array} 
	\right\}
\end{align*}
	...
	\for{\kappa} \semI[\lenv]{I}(\ca{}) ==
		...
		\begin{array}{@{}l@{\ }|@{\ }r@{}}
			...
			\begin{array}{@{}l@{}}
				...
				\quad\ca{} \in \semK[\lenv]{\capAss{}} \land\exsts{r, \amod{}, \amod{L}}\\
				\quad(p, p \composeL r, \amod{}, \amod{L}) \in \intermediateSem[\lenv\text{[} \vec{y} \mapsto \vec{v} \text{]}]{P} \land\null\\
				\quad(q, q \composeL r, \amod{}, \amod{L}) \in \intermediateSem[\lenv\text{[} \vec{y} \mapsto \vec{v} \text{]}]{Q} 
			\end{array}
		\end{array}
	\right\}
\]
...
\begin{array}{l l l}
	...
	\hspace*{-0.2cm} & \hspace*{-0.2cm} 
	& \hspace*{-0.1cm} 	
	\null\land \contains{\dom{\amod{L}}}{\capPart{r}}
\end{array}
\]
%
...
\]
%
The above states that the $(\amod{}, \amod{L})$ pair is closed under $(l, r, \amod{}')$ if the closure relation holds for any number of steps $n \in \Nats$ where 1) $l$ denotes the subjective view of the shared state, 2) $r$ denotes the context, 3) $l \composeL r$ captures the entire shared state  and 4) a step corresponds to the occurrence of an action as prescribed in either the current action model $\amod{}'$ or $\amod{L}$. The relation is satisfied trivially for no steps ($n = 0$). On the other hand, for an arbitrary $n\in \Nats$ the relation holds iff:
\begin{enumerate}
...
\end{definition}
%
As illustrated above, the global behaviour $\amod{}$ relies on the division between the subjective state $l$ and the context $r$. As such, it is essential to calculate it upon defining the semantics of $\shared{P}{I}$ when the boundary between the subjective state described by $P$ and the context is discernible.  In other words, we cannot correctly identify the global action model associated with  a shared state $s$ and a localised action model $\amod{L}$ without knowledge of the division between the subjective state and the context. 
