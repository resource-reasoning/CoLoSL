...
and $l_s$ are \emph{logical states} and $\amod{}$ and $\amod{\ell}$ are
...
As mentioned in the beginning of the section, we require that actions
in the local and global action models are \emph{contained} by the
shared state.

\begin{definition}[Action containment]
  An action $a$ is \emph{contained} by a logical state $l$, written
  $l\containI a$, if
  \[
  \m{fst}(a)\meetL l \neq\emptyset => \m{fst}(\updateFP{a})\leq l
  \]
\end{definition}

Note that only the \emph{active} precondition of $a$ has to be
contained in $l$. This is because actions may anticipate future
additions to the shared state, and require them as catalysts (as in
our set example~\ref{sec:set}).

Requiring that all actions are contained within the shared state is
not enough, since the shared state is subject to interferences by
these same actions. Rather, we will require that all possible
\emph{futures} of the shared state according to the action models
contain the actions. To define this, we start by defining the effect
of actions on the shared state.
...
We are now ready to define our containment condition on actions.
...
\begin{definition}[Locally-fenced action model]
  ...
  $\fence{} \in \pset{\LStates}$, written $\fence{} \fences \amod{}$,
  if, for all $l \in \fence{}$ and all $\ca{}\in\m{rg}(\amod{})$,
\[
\begin{array}{L}
  l\containI a /|
...
\begin{definition}[Action model containment]
  An action model $\amod{}$ is \emph{contained} by a logical state
  $l$, written $l\containI\amod{}$, if there exists a fence $\fence{}$
  such that $l\in\fence{}$ and $\fence{}\strictfences{}\amod{}$.
\end{definition}



A world $(l,l_s,\amod{},\amod{\ell})$ is well-formed if $l$ and $l_s$
are compatible, the capabilities found in $l\composeL l_s$ exist in
the action models, actions in the global action model correspond to
actions in $\amod{\ell}$, and $\amod{\ell}$ is contained by $l_s$.
...
\begin{definition}[Well-formedness]
  ...
  \begin{array}{L}
    ...
    \updateFP{a} = \updateFP{a'}) /| l\containI \amod{\ell}
  \end{array}
  \]
\end{definition}

Note that we also have that $l\containI \amod{}$.
...
$|=$ between worlds and formulas. It is parametric in the forcing
relation $|=_K$ for capability assertions, which we instantiate below
for tokens. We use two auxiliary forcing relations. The first one,
written $\slsat$, interprets \emph{atomic} formulas $\p{A}$ in the
usual separation logic sense over a single logical state, while the
other $|=_{l,\amod{},\amod{\ell}}$ interprets assertions over a single
...
\begin{definition}[Assertion semantics]
  ...
  $\semI[(.)]{.}: \LEnv --> \AMods$ denotes the semantics of
