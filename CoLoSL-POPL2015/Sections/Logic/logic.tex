...
\fig\ref{fig:concurrentInc}, the variables \li{x}, \li{y}, \li{z} at the
beginning of $\mathbb{INC}$). The \emph{shared logical state}, or shared
...
We elaborate on the need for both local and global action models in \S~\ref{subsec:local-global-am}. 
...
existing actions cannot affect future extra pieces of shared state. 
Similarly, we require that the new actions associated with newly
shared state (through \extendRule) are confined to that extension in the same sense, hence
...
thread means that the environment can perform the action.
%
...
be instantiated with \emph{any} separation algebras\footnote{In
  general, capabilities can be fractionally owned, in which case
  ...
  development. See the technical report for the general case.} (\textit{i.e.}
cancellative partial commutative monoids~\cite{asl}) satisfying the \emph{cross-split} property\footnote{
A separation algebra $(\mathbb{A}, +, \mathbf{1})$ satisfies the cross-split property iff for all $a, b, c, d \in \mathbb{A}$ where $a + b = c + d$, there exists $ac, ad, bc, bd \in \mathbb{A}$ s.t.
%
\vspace{-3pt}
\[
	a = ac + ad /| b = bc + bd /|
...
%\vspace{-5pt}
%\[
%	\begin{array}{@{} l @{\hspace*{3pt}} l @{\hspace{3pt}} l @{}}
%		a + b = c + d \iff &
%		\exsts{ac, ad, bc, bd} & 
%		 a = ac + ad /| b = bc + bd \;/| \\
%		 && c = ac + bc /| d = ad + bd
...
We write $\unitAM$ for the action model with empty domain.
...
We start by characterising the active and passive parts of
actions, then move on to define the effect of actions. This will
enable us to define our confinement condition and the set of
well-formed worlds.
...
\begin{definition}[Active part of an action]
  ...
  $\updateFP{(p,q)}$ is defined as the pair $(p', q')$ such that: \vspace{-10pt}
%
  \[
...
The compatibility of a shared state $g$ and an action pre-state $p$ is defined using the following notion of \emph{intersection} of logical states.
...
the set notation.\footnote{In general, separation algebras
  may lack the \emph{disjointness} property, in which case
  intersection between two states may yield more than one state. Thus,
  in our general definitions we use the set notation. Since in this paper our logical states satisfy disjointness, we omit the set notation in our exposition.}
...
may not affect $g$, \textit{i.e.}, its intersection with $g$ may be empty. In that case, we find that even though that action is potentially enabled, we do not need to take it into account since it leaves $g$ unchanged. We thus introduce the following notion of \emph{visible actions}.
...
states $\Heaps$ and capabilities $\Caps$. In this paper, we use standard heap and stack assertions as machine state assertions; and instantiate capability assertions with tokens. 
...
sense (ignoring the boxed assertions), and $I$, $\lmod$, and $\gmod$ \emph{agree} given the
decomposition $s$, $r$, in the following sense:
\begin{enumerate}
\item
  every action in $I$ is contained in $\lmod{}$;
\item
...
variables $\lenv\in\LEnv == \set{LVar} --> \set{Val} \uplus \set{Token}$ and a formula
...
\begin{definition}[Action model closure]
  ...
  written $\extendsAM{\lmod, \gmod}{s}{r}{\lmod'}$, if $\lmod' \subseteq \lmod$ ({\it cf. } property 1. in \defin~\ref{def:assertions} ) and 
...
\shared{P ** Q}{I} => 
\shared{P * \m{true}}{I} => 
\shared{P}{I}   \vspace{-0.5ex}
\]
%
\subsection{On the Need for Local and Global Action Models}\label{subsec:local-global-am}
We illustrate the need for both local and global action models in worlds through the following examples. \ex~\ref{ex:globalAM} showcases subtle reasoning about subjective views enabled by our logic thanks to global action models; example~\ref{ex:localAM} motivates the need for local action models.

\begin{example}\label{ex:globalAM}
%  This example showcases subtle reasoning about subjective views
%  enabled by our logic thanks to the global action model.  
...
\begin{example}\label{ex:localAM}
