...
%
\[
	\LAssertions \ni B ::= A \mid B \land B \mid B \lor B \mid \exsts{x} B \mid B * B \mid B \sepish B
\]
%
The \emph{flat assertions} $\FAssertions$, are defined by the following grammar. 
%
\[
	\FAssertions \ni \fass{P}, \fass{Q} ::= B \mid \shared{B}{I} \mid \fass{P} \land \fass{Q} \mid \fass{P} \lor \fass{Q} \mid \exsts{x} \fass{P} \mid \fass{P} * \fass{Q} \mid \fass{P} \sepish \fass{Q} 
\]
%
...
\begin{align*}
	\un{A} \eqdef & A\\
%
	\un{P \oplus Q} \eqdef & \un{P} \oplus \un{Q}  \quad \text{for } \oplus \in \{\land, \lor, *, \sepish\}\\
%
	\un{\exsts{x} P} \eqdef & \exsts{x} \un{P} \\
%
	\un{\shared{B}{I}} \eqdef& \shared{B}{I}\\
%
...
The \emph{unboxing} function $\unbox{.}: \FAssertions \rightarrow \LAssertions$, is defined as follows. 
%
\[
	\unbox{\fass{P}} = \overline{\exists{x}}^{x \in S}.\; B_1 * B_2 \text{ where } \ub{\fass{P}} = (S, B_1, B_2)
\]
%
where the auxiliary function $\ub{.}: \FAssertions \rightarrow (\pset{\textsf{LVar}} \times \LAssertions \times \LAssertions)$ is defined inductively as follows assuming that the existentially bound variables of the left-hand assertion are renamed to be distinct in a capture-avoiding manner. 
%
\begin{align*}
	\ub{B} \eqdef & (vars(B), B, \emp)\\
%
	\ub{\shared{B}{I}} \eqdef & (vars(B), \emp, B)\\
%
	\ub{\fass{P} \oplus \fass{Q}} \eqdef & \left(S \uplus S', B_1 \oplus B'_1, B_2 \sepish B'_2 \right)\\
	& \text{ where } \ub{\fass{P}} = (S, B_1, B_2) \text{ and } \ub{\fass{Q}} = (S', B'_1, B'_2)\\
	& \text{ and } \oplus \in \{\land, *, \sepish\}\\
%
	\ub{\fass{P} \lor \fass{Q}} \eqdef & \left(S \uplus S', B_1 \lor B'_1, B_2 \lor B'_2 \right)\\
	& \text{ where } \ub{\fass{P}} = (S, B_1, B_2) \text{ and } \ub{\fass{Q}} = (S', B'_1, B'_2)\\
%
	\ub{\exsts{x} \fass{P}} \eqdef& (S \uplus \{x\}, B_1, B_2) \text{ where } \ub{\fass{P}} = (S, B_1, B_2)
\end{align*}
\end{definition}
%
%
...
%
	\ers{\fass{P} \oplus \fass{Q}} \eqdef & \ers{\fass{P}} \oplus \ers{\fass{Q}} \text{ where } oplus \in \{\land, \lor, *, \sepish \}\\
%
...
\begin{mathpar}
	...
	{
		\fenceAss{} \sepish P \slentails \m{false}
	}
	...
	}
	{
		...
		&
		\fenceAss{} \sepish P \slentails \fenceAss{}' \sepish P
		&
		\fenceAss{}' \strictfences \left\{\capAss{}: P \swap Q \right\}		
	}
...
%	}
	...
	{
		\fenceAss{} \sepish P \slentails \fenceAss{}
		&
		\precise{\fenceAss{}}
		&
		P \cap Q \slentails \emp
		&
		\left(P \septraction \fenceAss{} \right) * Q \slentails \fenceAss{}	
	}
...
\end{mathpar}
\hrule
\caption{Local fencing judgements where the separation algebra of logical states satisfies the disjointness property.}
...
%
	...
	}
	{
		P \cap Q \slentails \emp	
		&
		\left(P \septraction \fenceAss{} \sepish P \right) * Q \slentails \fenceAss{}
	}
	...
	}
	{
		\fenceAss{}' \slentails \fenceAss{}
		&
		P \sepish \fenceAss{} \slentails P \sepish \fenceAss{}'
		&
		\fenceAss{}' \fences \left\{\capAss{}: P \swap Q \right\}		
	}
...
\begin{mathpar}
	...
	\infer[\proofRule{Disj-L}]{
		\bigcup_{i \in I} \left\{\capAss{} : P_i \swap Q \right\}
		\approx^{\m{true}}
		\left\{\capAss{}: \left( \bigvee_{i \in I} P_i \right) \swap Q \right\} 
	}
	{
	}
	...
	\infer[\proofRule{Disj-R}]{
		\bigcup_{i \in I}  \left\{ \capAss{}: P \swap Q_i \right\}
		\approx^{\m{true}}
		\left\{\capAss{}: P \swap \left( \bigvee_{i \in I} Q_i \right) \right\} 
	}
	{
	}
	
	\infer[\proofRule{Exist}]{
		\left\{ \capAss: \overline{\exsts{v_i \in S_i}}^{i \in I} P \swap Q \right\} 
		\approx^{\m{true}}
	...
	\infer[\proofRule{Hide}]{
		\left\{ \capAss{}: P * R   \swap Q * R \right\} \weakenI{\fenceAss{}} 
		\emptyset 	
	}{	
		\exact{R}
		&
...
%	}
	...
	\infer[\proofRule{False-L}]{	
		\left\{ \capAss{}: P \swap Q \right\} \weakenI{\fenceAss{}} 
		\emptyset 	
	}{
%		& \fenceAss{} \fences  \left\{ \capAss{}: \left\{ P \swap Q \right\}\right\}
		\fenceAss{} \sepish P \slentails \m{false}
...
%		& \fenceAss{} \fences  \left\{ \capAss{}: \left\{ P \swap Q \right\}\right\}
		\left(P \septraction (P \sepish \fenceAss{})\right) * Q \slentails \m{false}
	}
...
%	}
	...
	\infer[\proofRule{Expand/Contract}]{
		\left\{ \capAss{}: P \swap Q\right\} \;\approx^{\fenceAss{}}\;  \bigcup_{i \in I} \left\{ \capAss{}: P * R_i \swap Q * R_i \right\}	
	}
	{
		\fenceAss{} \sepish P \slentails \bigvee_{i \in I} \fenceAss{} \sepish \left(P * R_i \right)
		&
...
%	
	...
	{
		\stableIn{B}{I}{\unbox{\fass{R} * \shared{B}{I}}}
	}
\end{mathpar}\vspace{5pt}\\
%
%
...
%	
	...
	\infer{
		\stableIn{B}{\left\{\interAss{\capAss{}}{\bar{y}}{Q_1}{Q_2}\right\}}{B'}	
	}
	{
		\capAss{} * B' \slentails \m{false}
	}
	
	
	\infer{
		\stableIn{B}{\left\{\capAss{}: P \swap Q \right\}} {B'}	
	}
	{
		B \sepish \erase{P} \slentails \m{false}
	}
	...
	{
		\left(\erase{P} \septraction (\erase{P} \sepish B)\right) * \erase{Q} \slentails B * \m{true}
	}
...
\hrule
%\vspace*{5pt}
\caption{Stability judgements where $P, Q, R \in \Assertions$; $\fass{P}, \fass{Q}, \fass{R} \in \FAssertions$; and $B, B', B'' \in \LAssertions$.}
